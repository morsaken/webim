...
class Arr {
  ...
  public static function get($target, $key, $default = null) {
    if (is_null($key) || trim($key) == '') return $target;
  ...
  public static function first($array, $callback = null, $default = null) {
    foreach ($array as $key => $value) {
      if (!$callback instanceof Closure || call_user_func($callback, $key, $value)) return $value;
    }
   ...
   * @return string
   */
  public static function to($array, $as = 'json') {
    //Return
    $text = '';

    if (!is_scalar($array)) {
      //Set "as" to default
      if (!in_array($as, array('json', 'xml', 'plain'))) {
        $as = 'json';
      }
      ...
      switch ($as) {
        ...
        case 'xml':
          ...
          foreach ((array)$array as $tag => $value) {
            ...
            if (is_scalar($value)) {
              $text .= '<' . $tag . $params . '><![CDATA[' . $value . ']]></' . $tag . '>';
            } else {
              $text .= '<' . $tag . $params . '>' . static::to($value, $as) . '</' . $tag . '>';
            }
          }
   ...
   * @return string
   */
  public static function str($array, $glue = ' ') {
    //Return
    $str = '';
    ...
    if (is_array($array) || is_object($array)) {
      ...
      foreach ($array as $key => $value) {
        if (is_scalar($value)) {
          //String, int
          $strs[] = $key . ': ' . (is_numeric($value) ? $value : '"' . $value . '"');
        } elseif (is_array($value) || is_object($value)) {
          //Set array again
          $strs[] = static::str($value, $glue);
        } else {
          //Null
          $strs[] = $key . ': null';
        }
