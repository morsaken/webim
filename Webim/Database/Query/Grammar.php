...
class Grammar extends BaseGrammar {
  ...
  protected function compileJoins(Builder $query, $joins) {
    ...
    foreach ($joins as $join) {
      ...
      $sql[] = strtoupper($type) . ' JOIN ' . $table . (count($clauses) ? ' ON (' . implode(' ', $clauses) . ')' : '');
    }
  ...
  protected function compileJoinConstraint(array $clause) {
    ...
    if (isset($clause['where']) && $clause['where']) {
      $second = '?';
    } elseif (is_array($clause['second'])) {
      $second = '(' . $this->parameterize($clause['second']) . ')';
    } elseif (!is_null($clause['second'])) {
      $second = $this->wrap($clause['second']);
    }

    return trim(strtoupper($clause['boolean']) . ' ' . $first . ' ' . $clause['operator'] . ' ' . $second);
  }
  ...
  protected function compileAggregate(Builder $query, $aggregate) {
    ...
    return 'SELECT ' . strtoupper($aggregate['function']) . '(' . $column . ') AS ' . $this->columnize(array('aggregate'));
  }
  ...
  public function compileSelect(Builder $query) {
    if (is_null($query->columns) || !count($query->columns)) $query->columns = array('*');
  ...
  protected function whereBetween(Builder $query, $where) {
    ...
    $values = $this->parameter($where['values'][0]) . ' AND ' . $this->parameter($where['values'][1]);

    return $this->wrap($where['column']) . ' ' . strtoupper($between) . ' ' . $values;
  }
  ...
  protected function compileHavings(Builder $query, $havings) {
    ...
    return 'HAVING ' . preg_replace('/and /i', '', $sql, 1);
  }
  ...
  protected function compileHaving(array $having) {
    ...
    if ($having['type'] === 'raw') {
      return strtoupper($having['boolean']) . ' ' . $having['sql'];
    }
  ...
  protected function compileBasicHaving($having) {
    ...
    return strtoupper($having['boolean']) . ' ' . $column . ' ' . $having['operator'] . ' ' . $parameter;
  }
